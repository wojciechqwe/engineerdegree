    ...
    if (this.state.showQuestions) {
      ...
      for (let questionID of point.questions) {
        renderedQuestions.push(
          <div key={this.questions[questionID].id} style={this.styles.conversationCloud} onClick={() => { this.readAnswer(this.questions[questionID]) }}>
            {this.questions[questionID].question[0]}
          </div>
        )
      }
  ...
  render() {
    ...
    return (
      ...
      <div style={style.character}>
        {this.showQuestions(this.points[this.state.currentPoint])}
      </div>
